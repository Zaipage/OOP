  ...
  // TODO: Implementasi class Member dengan exception handling
    ...
    public Member(String name, String memberId, int maxBorrowLimit) throws LibraryException{
      if (name == null || name.trim().isEmpty()){
        throw new LibraryException("Name cannot be null", "INVALID_MEMBER");
      }

      if (memberId == null || memberId.trim().isEmpty()){
        throw new LibraryException("Member Id cannot be null", "INVALID_MEMBER");
      }
    
      if (maxBorrowLimit <= 0){
        throw new LibraryException("Borrow limit have to be more than 0", "INVALID_MEMBER");
      }
    ...
    public void borrowBook(String bookTitle) throws LibraryException {
      ...
      if (borrowedBooks.contains(bookTitle)){
        // kalau di list borrowedBook ada titlenya tandanya buku sudah dipinjam
        // dan tandanya bukunya gabisa dipinjam lagi
      ...
      if (borrowedBooks.size() >= maxBorrowLimit){
        // borrowed.size() digunakan untuk mengetahui isi list skrg alias sudah berapa buku dipinjam

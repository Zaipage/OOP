...
public class InvalidBookException extends LibraryException {
    ...
    public InvalidBookException(String message){
        // kalau cuma mau nampilin error message maka parameternya cukup message
        super(message,"INVALID_BOOK");
    }

    public InvalidBookException(String message, Throwable cause){
        // kalau mau chaining atau nyambung ke error lain makan parameternya tambahin 'Throwable cause'
    ...
    public String getMessage(){
        String baseMessage = String.format("[INVALID_BOOK] %s", super.getMessage());
        if (bookTitle != null){
            if (invalidValue != null){
                baseMessage += String.format(" (Book: %s, Invalid Field: %s, Value: '%s')", bookTitle, invalidField, invalidValue);
            } else {
                baseMessage += String.format(" (Book: %s, Invalid Field: %s)", bookTitle, invalidField);
            }
        }
        return baseMessage;
